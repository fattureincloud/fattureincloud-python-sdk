...
class TestReceiptsApi(unittest.TestCase):
    ...
    def test_get_receipt_pre_create_info(self):
        resp = {
            'status': 200,
            'data': b'{"data": {"rc_centers_list": ["bg", "mi"], "payment_accounts_list": [{"id": 1, "name": "bank"}], "categories_list": ["cat5", "cat6"], "vat_types_list": [{"value": 22.0}]}}',
            'reason': "OK"
        }
        ...
        self.api.api_client.rest_client.GET = unittest.mock.MagicMock(return_value = mock_resp)
        expected = GetReceiptPreCreateInfoResponse( data=ReceiptPreCreateInfo( rc_centers_list=[ "bg", "mi" ], payment_accounts_list=[ PaymentAccount( id=1, name="bank" ) ], categories_list=[ "cat5", "cat6" ], vat_types_list=[ VatType( value=22.0 ) ] ) )
