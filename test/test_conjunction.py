...
class TestConjunction(unittest.TestCase):
    ...
    def testConjunction(self):
        ...
        assert right == conjunction.get_right()
        assert "(team = 'Volley Bergamo 1991' and age >= 18)" == conjunction.build_query()
        ...
        assert "(team = 'Volley Bergamo 1991' and best_player is not null)" == conjunction.build_query()

    def testEquals(self):
        """Test Equals"""
        conjunction1 = Conjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.EQ, value="1"))
        conjunction2 = Conjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.EQ, value="1"))
        assert conjunction1 == conjunction2

        conjunction3 = Conjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo"), Condition(field="position", op=Operator.EQ, value="1"))
        assert conjunction1 != conjunction3

        conjunction4 = Conjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.LT, value="1"))
        assert conjunction1 != conjunction4

        disjunction = Disjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.EQ, value="1"))
