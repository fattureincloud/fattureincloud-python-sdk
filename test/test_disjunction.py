...
class TestDisjunction(unittest.TestCase):
    ...
    def testDisjunction(self):
        ...
        assert right == disjunction.get_right()
        assert "(team = 'Volley Bergamo 1991' or age >= 18)" == disjunction.build_query()
        ...
        assert "(team = 'Volley Bergamo 1991' or best_player is not null)" == disjunction.build_query()

    def testEquals(self):
        """Test Equals"""
        disjunction1 = Disjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.EQ, value="1"))
        disjunction2 = Disjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.EQ, value="1"))
        assert disjunction1 == disjunction2

        disjunction3 = Disjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo"), Condition(field="position", op=Operator.EQ, value="1"))
        assert disjunction1 != disjunction3

        disjunction4 = Disjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.LT, value="1"))
        assert disjunction1 != disjunction4

        conjunction = Conjunction(Condition(field="team", op=Operator.EQ, value="Volley Bergamo 1991"), Condition(field="position", op=Operator.EQ, value="1"))
